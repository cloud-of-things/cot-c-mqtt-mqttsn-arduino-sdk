...
* upload the content of [smartrest_templates.csv](./smartrest_templates.csv) - that is, copy and paste the text into the input field below the \[upload Template\] button, set the name **`CoT-MQTT-Arduino_Lib`** into the field at the left of the \[upload Template\] button and finally press that button.
