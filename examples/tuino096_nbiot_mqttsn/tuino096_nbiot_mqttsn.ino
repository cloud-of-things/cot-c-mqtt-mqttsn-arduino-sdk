...
#define NB_IOT_MQTTSN_PWD		"WXBTHn2D"
...
unsigned long loopInterval = 30000; //ms
...
void loop() {
	...
	if (!lastLoopStart || loopStart > (lastLoopStart + loopInterval)) {
		...
		for (int i=0; i<3; ++i) {
			sendResult = cotDevice.sendMeasurement("ty", "Room1", "Temperature", "C", temp, now());
			if ( ! sendResult) {
				++attempts;
			}else{
				break;
		...
		if(attempts == 3) { // reset modem
			DT_DEBUG_PRINTLN_INFO("Loop[%lu] RESET", loopCount);
			setup();
		}
